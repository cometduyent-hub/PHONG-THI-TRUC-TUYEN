          ...
          {studentViewTab === "lookup" ? (
            ...
            <div style={{ padding: "10px 0" }}>
              ...
              <div style={{ display: "flex", gap: "8px", margin: "20px 0", flexWrap: "wrap", alignItems: "center" }}>
                <span style={{ fontSize: "13px", fontWeight: "700", color: "#0f766e" }}>Lọc kết quả:</span>
                {(["all", "correct", "incorrect", "unanswered"] as const).map(f => (
                  <button 
                    ...
                    style={{ 
                      ...
                      background: reviewFilter === f ? "#0d9488" : "#f8fafc",
                      color: reviewFilter === f ? "#fff" : "#334155",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                {submissionDetails
                  ...
                  .map((item, idx) => (
                    <div key={item.question.id} style={{ background: "#f8fafc", padding: "16px", borderRadius: "8px", border: `1px solid ${item.question.section === 'ESSAY' ? '#cbd5e1' : (item.isCorrect ? '#86efac' : (item.isUnanswered ? '#cbd5e1' : '#fca5a5'))}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                        <span style={{ fontWeight: "700", color: "#0f766e" }}>Câu {idx + 1} ({sectionLabel[item.question.section]})</span>
                        <span style={{ fontSize: "12px", fontWeight: "700", color: item.isCorrect ? "#059669" : "#dc2626" }}>
                          {item.question.section === "ESSAY" ? "Đã gửi tự luận" : (item.isCorrect ? "✓ Chính xác" : "✗ Chưa chính xác")}
                        </span>
                      </div>
                      <div style={{ fontSize: "13px", color: "#334155", marginBottom: "8px" }}>{item.questionText}</div>
                      <div style={{ background: "#fff", padding: "10px", borderRadius: "6px", fontSize: "13px", border: "1px solid #e2e8f0", display: "flex", flexDirection: "column", gap: "4px" }}>
                        <div>- Bạn đã chọn / trả lời: <b style={{ color: "#0284c7" }}>{item.studentAnswer}</b></div>
                        {item.question.section !== "ESSAY" && <div>- Đáp án đúng: <b style={{ color: "#059669" }}>{item.correctAnswer}</b></div>}
                      </div>
                    </div>
                  ))}
              </div>
